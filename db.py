...
class DB_Connector():
    ...
    def upload_csv(self, filepath):
        try:
            csv_file = pd.read_csv(filepath,delimiter=',',encoding='gbk')
        except:
            csv_file = pd.read_csv(filepath, delimiter=',', encoding='utf-8')
        for index in csv_file.index:
            try:
                db.curse.execute("insert into %s (name,class1,class2,date,data) values%s"%(self.table,str(tuple(map(str,csv_file.loc[index,:])))))
            except sqlite3.IntegrityError:
                db.curse.execute("update "+self.table+ " set name='%s',class1='%s',class2='%s',date='%s',data='%s'"%tuple(map(str, csv_file.loc[index, :]))+
...
if __name__ == '__main__':
    ...
    db = DB_Connector()
    import os
    for ff in os.listdir('requirement'):
        db.upload_csv("requirement/%s"%ff)
    db.close()
